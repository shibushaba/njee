-- Only the receiver may count opens for once/twice; sender cannot burn view budget.
...
begin
  ...
  if lim is not null and lim > 0 then
    if auth.uid() = m.sender_id then
      return jsonb_build_object('ok', false, 'reason', 'sender_cannot_open', 'unlimited', false);
    end if;
    if auth.uid() is distinct from m.receiver_id then
      return jsonb_build_object('ok', false, 'reason', 'forbidden');
    end if;
  end if;
